...
export class PostsService {
                ...
                throw new HttpException(
                    {
                        ...
                        error: `there is already post with this name`,
                    },
                    HttpStatus.CONFLICT,
                );
            }
    ...
    async deactivate(postId: number): Promise<Post> {
        try {
            const user = this.request.user;
            const post = await this.postDataBaseService.deactivate(
                postId,
                user.id,
            );
            if (!post) {
    ...
    async update(
        ...
        try {
            const user = this.request.user;
            const post = await this.postDataBaseService.updatePost(
                updatePostDto,
                postId,
                user.id,
                images,
    ...
    async remove(postId: number): Promise<Post> {
        try {
            const user = this.request.user;
            const post = await this.postDataBaseService.removePost(
                postId,
                user.id,
            );
            if (!post) {
