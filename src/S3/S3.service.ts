...
export class S3Service {
    amazonS3ObjectOrl =
        'https://olx-amazing-clone.s3.eu-central-1.amazonaws.com/';
    s3 = new AWS.S3({
    ...
    async update(
        ...
        const imagesNames = images.map(
            (images) => `${postId}-${images.filename}`,
        );
        const deleteImages = postImages.filter(
            (image) => !imagesNames.includes(image),
        );
        await Promise.all(
            deleteImages.map(async (image: string) => {
                const uploadParams = {
                    Bucket: process.env.AWS_BUCKET_NAME,
