...
export class AuthService {
    ...
    async login(userDto: CreateUserDto): Promise<Token> {
        const user = await this.userDataBaseService.findUserByEmail(
            userDto.email,
        );
        if (!user) {
    ...
    async registration(userDto: CreateUserDto): Promise<User> {
        const candidate = await this.userDataBaseService.findUserByEmail(
            userDto.email,
        );
        if (candidate) {
            throw new HttpException(
    ...
    async activation(activationLink: string): Promise<ActivationMessage> {
        const user = await this.userDataBaseService.findUserByActivationLink(
            activationLink,
        );
        if (!user) {
            throw new HttpException(
